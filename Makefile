# This Makefile is for the Astro::Units extension to perl.
#
...
#     CONFIGURE_REQUIRES => {  }
#     NAME => q[Astro::Units]
#     PREREQ_PM => { Carp=>q[0], Math::BigFloat=>q[0], Test::More=>q[0], bignum=>q[0] }
#     TEST_REQUIRES => {  }
#     VERSION_FROM => q[lib/Astro/Units.pm]
#     test => { TESTS=>q[t/*.t] }
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Astro/Units.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIBDEP)$(DFSEP)Config.pm $(PERL_INCDEP)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Astro
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Astro
...
TO_INST_PM = lib/Astro/Units.pm

PM_TO_BLIB = lib/Astro/Units.pm \
...
DISTNAME = Astro-Units
DISTVNAME = Astro-Units-1.00
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=$(MAN3EXT) --perm_rw=$(PERM_RW) -u \
	  lib/Astro/Units.pm $(INST_MAN3DIR)/Astro::Units.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: Astro-Units' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '  bignum: '\''0'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Astro-Units",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '            "bignum" : "0"' >> META_new.json
	$(NOECHO) $(ECHO) '         }' >> META_new.json
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="bignum::" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Astro/Units.pm blib/lib/Astro/Units.pm 
